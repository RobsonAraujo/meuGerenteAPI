...
// Example get
exports.compareCompetitive = function (req, res) {
    ...
    return insightDao.compareCompetitive(id, segment).then(result => {

        if (!result || result.length <= 0) {

            winston.info('InsightsDao-> compareCompetitive *Successfully -  not result*')
            return res.status(200).json({
                status: 200,
                message: "Successfully -  not result"
            })
        } else {
            winston.info('InsightsDao-> compareCompetitive *The request is OK*')
            return res.status(200).json({
                status: 200,
                message: "The request is OK ",
                data: result
        ...
        winston.error(`InsightsDao-> compareCompetitive *${error}*`)
        return res.status(500).json({
            status: 500,
            message: "Internal Server error - " + error
        })
